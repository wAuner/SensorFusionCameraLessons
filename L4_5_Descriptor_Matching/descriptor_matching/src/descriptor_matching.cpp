                      ...
                      vector<cv::DMatch> &matches, string descriptorType, string matcherType, string selectorType)
{
    ...
    { // k nearest neighbors (k=2)       
        ...
        for (auto knnMatchIt = knnMatches.begin(); knnMatchIt != knnMatches.end(); ++knnMatchIt){
            float distanceRatio = (*knnMatchIt)[0].distance / (*knnMatchIt)[1].distance;
            if (distanceRatio < minDistanceRatio) {
                matches.push_back((*knnMatchIt)[0]);
            }
        }
